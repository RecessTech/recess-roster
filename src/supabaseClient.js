...
export const db = {
  ...
  // Rotate the org's public Transfer Hub-share token (invalidates any previously shared link).
  async regenerateOrgTransferPublicToken(orgId) {
    const { data, error } = await supabase
      .from('organisations')
  ...
  // Generic metrics store -- one row per (tab, section, metric), each holding
  // its whole weekly time series as a `{ "YYYY-MM-DD": value }` JSONB object.
  // Mirrors the source analytics spreadsheet's own tabs (Revenue / Costs /
  // Customer / Budget) exactly rather than modelling each metric as a bespoke
  // column, so every KPI the sheet ever tracked lives in R-Shift without a
  // schema change per metric.
  async getAnalyticsMetrics(orgId, tab) {
    const { data, error } = await supabase
      .from('analytics_metrics')
      .select('section, metric, series')
      .eq('org_id', orgId)
      .eq('tab', tab);
    if (error) throw error;
    return data || [];
